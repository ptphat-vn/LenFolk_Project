...
exports.getAll = async (req, res, next) => {
  ...
  const docs = await Permission.find(JSON.parse(queryStr)).sort(req.query.sort ? req.query.sort.split(',').join(' ') : '-createdAt').skip(skip).limit(limit).select('-__v');
...
exports.getOne = async (req, res, next) => {
  try {
  const doc = await Permission.findById(req.params.id);
...
exports.updateOne = async (req, res, next) => {
  try {
  const doc = await Permission.findByIdAndUpdate(req.params.id, req.body, { returnDocument: 'after', runValidators: true });
  if (!doc) return res.status(404).json({ success: false, message: 'Không tìm thấy dữ liệu' });
