...
exports.getAll = async (req, res, next) => {
  ...
  const docs = await ModeratorLog.find(JSON.parse(queryStr)).sort(req.query.sort ? req.query.sort.split(',').join(' ') : '-createdAt').skip(skip).limit(limit).select('-__v');
...
exports.getOne = async (req, res, next) => {
  try {
  const doc = await ModeratorLog.findById(req.params.id);
...
exports.createOne = async (req, res, next) => {
  try {
  const doc = await ModeratorLog.create(req.body);
...
exports.updateOne = async (req, res, next) => {
  try {
  const doc = await ModeratorLog.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
  if (!doc) return res.status(404).json({ success: false, message: 'No document found with that ID' });
...
exports.deleteOne = async (req, res, next) => {
  try {
  const doc = await ModeratorLog.findByIdAndDelete(req.params.id);
  if (!doc) return res.status(404).json({ success: false, message: 'No document found with that ID' });
